...
function LogRow({ item, index }) {
  ...
  return (
    <div
      data-testid="log-row"
      className="flex items-start gap-4 px-7 py-5 border-b transition-colors"
      style={{
        borderColor: 'rgba(3,3,82,0.07)',
        backgroundColor: isId ? 'rgba(3,3,82,0.018)' : 'transparent',
    ...
    >
      ...
      <div
        ...
        style={{
          backgroundColor: isId ? 'rgba(3,3,82,0.10)' : '#f0ede8',
          color: isId ? '#030352' : '#9898aa',
        }}
      >
        {isId
          ? (item.contact_name?.[0] || item.contact_email?.[0] || '?').toUpperCase()
          : <User size={14} />}
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center flex-wrap gap-x-2 gap-y-0.5">
          ...
          <span
            ...
            style={{ color: '#0369a1', fontFamily: 'IBM Plex Mono, monospace' }}
            title={item.url}
          >
            {shortUrl(item.url)}
          </span>
      ...
      >
        ...
        <div className="relative z-10 flex items-start justify-between">
          <div>
            ...
            {last && <p className="text-base mt-1 font-semibold" style={{ color: 'var(--brand-navy)', opacity: 0.6 }}>Updated {timeAgo(last)}</p>}
          </div>
          <div className="flex items-center gap-4">
            ...
            <Button variant="ghost" size="sm" onClick={fetchLogs} className="h-9 w-9 p-0 rounded-lg" style={{ color: 'var(--brand-navy)', opacity: 0.6 }}>
              <RefreshCw size={14} />
            </Button>
          </div>
        </div>
      ...
      <div className="rounded-2xl border overflow-hidden" style={{ borderColor: 'var(--stroke)', backgroundColor: '#ffffff', boxShadow: '0 4px 20px rgba(3,3,82,0.06)' }}>
        {loading ? (
          ...
          <div data-testid="logs-feed">
            {logs.map((item, i) => <LogRow key={`${item.contact_id}-${i}`} item={item} index={i} />)}
          </div>
        )}
      </div>
    </div>
